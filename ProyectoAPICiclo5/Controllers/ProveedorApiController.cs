...
namespace ProyectoAPICiclo5.Controllers
{
    ...
    public class ProveedorApiController : ControllerBase
    {
        ...
        public async Task<ActionResult<Producto>> GetProveedorxId(int idProveedor)
        {
            var proveedor = await Task.Run(() => (new ProveedorDAO()).GetProveedorxId(idProveedor));
            return Ok(proveedor);
        }

        //eliminar proveedor
        [HttpDelete("EliminarProveedor")]
        public async Task<ActionResult<String>> EliminarProveedor(int idProveedor)
        {
            var mensaje = await Task.Run(() => (new ProveedorDAO()).EliminarProveedor(idProveedor));
            return Ok(mensaje);
        }
